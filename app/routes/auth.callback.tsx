...
// Shopify CLI automatically allow-lists /auth/callback for every Quick Tunnel
// session. This route handles only the signed external-store OAuth callback;
// embedded Shopify auth continues to use the library's /auth flow.
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  // The static route also receives Shopify's normal embedded-app callback.
  // Let the Shopify library complete that flow unless this is one of our
  // signed external-store authorization states.
  if (!state || !isExternalOAuthState(state)) {
